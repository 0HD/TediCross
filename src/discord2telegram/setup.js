...
function setup(logger, dcBot, tgBot, messageMap, bridgeMap, settings, datadirPath) {
	...
	dcBot.on("error", (err) => {
		if (err.code === "ECONNRESET") {
			...
			logger.warn("Lost connection to Discord's servers. The bot will resume when connection is reestablished, which should happen automatically. If it does not, please report this to the support channel");
		} else {
			// Unknown error. Tell the user to tell the devs
			logger.error("The Discord bot ran into an error. Please post the following error message in the support channel");
			logger.error(err);
		}
	});
